...
async def authenticate_user(email: EmailStr, password: str, session: AsyncSession):
    ...
    if not user or verify_password(plain_password=password, hashed_password=user.password) is False:
        db_logger.warning("Invalid email or password")
        return None
    return user
...
async def get_token(request: Request, token_type: TokenType):
    ...
    if not token:
        api_error_logger.error("token is not found")
...
async def get_current_user(request: Request, token_type: TokenType, session: AsyncSession):
    ...
    if (not expire) or (expire_time < datetime.now(timezone.utc)):
        api_error_logger.error(f"token for user with id - {user_id} is not found")
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='token has been expired')
    
    user_id = payload.get('sub')
    if not user_id:
        api_error_logger.error(f"user_id is not found in token")
    ...
    if not user:
        db_error_logger.error("user in not found")
...
async def get_current_admin_user(request: Request, session: AsyncSession):
    ...
    if current_user.role == "admin":
        api_logger.info(f"user with email - {current_user.email} is admin")
        return current_user
    api_logger.info(f"user with email - {current_user.email} is not admin")
